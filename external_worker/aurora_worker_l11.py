...
L11_SCHEMA_VERSION = "l11_symbol_ranking_inside_group_v1"
...
L11_INPUT_FIELDS = [
    ...
    "selection_runtime", "trade_permission", "entry_signal",
]
...
L11_RANKED_FIELDS = [
    ...
    "ranking_group_rank", "symbol", "canonical_symbol", "asset_class", "market_group", "market_segment", "l11_group_score",
    "ranking_group_rank_percentile", "rank_state", "leader_flag", "backup_flag", "backup_rank", "backup_reason", "top5_per_ranking_group",
    "risk_review_flag", "not_rankable_reason", "component_available_count", "component_missing_count", "weighted_available_average",
    "missing_layer_penalty", "stale_layer_penalty", "risk_review_penalty", "l6_cost_score", "l6_weight", "l6_state", "l7_session_score", "l7_weight", "l7_state",
    "l8_movement_score", "l8_weight", "l8_state", "l9_structure_score", "l9_weight", "l9_state", "component_summary", "reason", "meaning",
    "directional_validity", "expectancy_validated", "selection_runtime", "trade_permission", "entry_signal",
]

L11_TOP5_FIELDS = [
    "ranking_group", "ranking_group_slug", "group_state", "rankable_count", "top_rank", "symbol", "l11_group_score", "rank_state",
    "leader_flag", "backup_flag", "component_summary", "risk_review_flag", "reason", "source_ranked_symbols_checksum", "selection_runtime", "trade_permission", "entry_signal",
]
...
class L11PublishSummary:
    ...
    symbol_rank_filename_mode: str = "sanitized_symbol__payload_checksum"
...
def _manifest_text(name: str, row_count: int, payload_text: str, reason: str) -> str:
    return "\n".join([
        ...
        "selection_runtime=false", "trade_permission=false", "entry_signal=false", f"generated_utc={utc_stamp()}", f"generated_unix={unix_time()}", "",
    ])
...
def _component(layer_key: str, symbol: str, render_rows: Dict[str, Dict[str, Dict[str, str]]], render_manifest: Dict[str, str]) -> Dict[str, str]:
    ...
    return {
        ...
        "manifest_checksum": manifest_checksum, "manifest_status": manifest_status, "stale": "true" if stale else "false",
        "risk_review": "true" if risk_review else "false", "reason": "available" if available else "missing_or_unusable_score",
        "weight": str(int(L11_COMPONENT_WEIGHTS[layer_key])),
    }
...
def _score(components: Dict[str, Dict[str, str]], rank_state: str) -> Tuple[float, float, float, float, float, str]:
    ...
    for key in ("l6", "l7", "l8", "l9"):
        ...
        if item.get("stale") == "true": stale_penalty -= 8.0
        if item.get("risk_review") == "true": risk_penalty -= 5.0
    ...
    return final_score, average, missing_penalty, stale_penalty, risk_penalty, ";".join(parts)
...
def _rank_rows(rows: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for _group, members in sorted(groups.items()):
        ...
        for idx, row in enumerate(rankable, start=1):
            ...
            row["top5_per_ranking_group"] = "true" if idx <= 5 else "false"
            ...
            row["backup_reason"] = "top5_group_backup" if 1 < idx <= 5 else "not_backup"
            ranked.append(row)
        for row in members:
            if str(row.get("rank_state", "")).startswith("not_rankable"):
                row.update({"ranking_group_rank":"not_available", "ranking_group_rankable_count":str(rankable_count), "ranking_group_not_rankable_count":str(not_rankable_count), "ranking_group_symbol_count":str(len(members)), "ranking_group_rank_percentile":"not_available", "top5_per_ranking_group":"false", "leader_flag":"false", "backup_flag":"false", "backup_rank":"not_available", "backup_reason":"not_rankable"})
                ranked.append(row)
    return ranked

def _summary_text(summary: L11PublishSummary) -> str:
    return "\n".join([
        "schema_name=l11_symbol_ranking_inside_group_summary", f"schema_version={L11_SCHEMA_VERSION}", f"layer_id={L11_LAYER_ID}", f"layer_name={L11_LAYER_NAME}",
        f"owner={L11_OWNER}", f"authority={L11_AUTHORITY}", f"status={summary.status}", f"reason={summary.reason}", f"input_symbol_count={summary.input_symbol_count}",
        f"ranking_group_count={summary.ranking_group_count}", f"ranked_symbol_count={summary.ranked_symbol_count}", f"ranked_partial_count={summary.ranked_partial_count}",
        f"risk_review_count={summary.risk_review_count}", f"not_rankable_taxonomy_count={summary.not_rankable_taxonomy_count}", f"not_rankable_quality_count={summary.not_rankable_quality_count}",
        f"top5_group_count={summary.top5_group_count}", f"top5_symbol_count={summary.top5_symbol_count}", f"symbol_rank_files_written={summary.symbol_rank_files_written}", f"symbol_rank_files_actual={summary.symbol_rank_files_actual}",
        f"symbol_rank_filename_mode={summary.symbol_rank_filename_mode}", f"write_failed_count={summary.write_failed_count}", "component_weights=L6:25,L7:20,L8:25,L9:30", "input_taxonomy_source=L10", "input_surface_layers=L6,L7,L8,L9",
        "meaning=intra_group_inspection_priority_only", "directional_validity=false", "expectancy_validated=false", "selection_runtime=false", "trade_permission=false", "entry_signal=false",
        f"ranked_symbols_by_group_path={summary.ranked_symbols_by_group_path}", f"ranking_group_top5_path={summary.ranking_group_top5_path}", f"generated_utc={utc_stamp()}", f"generated_unix={unix_time()}", "",
    ])

def _symbol_rank_text(row: Dict[str, str]) -> str:
    data = dict(row, schema_name="l11_symbol_rank_sidecar", schema_version=L11_SCHEMA_VERSION, layer_id=L11_LAYER_ID, layer_name=L11_LAYER_NAME, owner=L11_OWNER)
    keys = ["schema_name", "schema_version", "layer_id", "layer_name", "owner", "symbol", "ranking_group", "ranking_group_rank", "ranking_group_rankable_count", "ranking_group_rank_percentile", "l11_group_score", "rank_state", "top5_per_ranking_group", "leader_flag", "backup_flag", "component_available_count", "l6_cost_score", "l7_session_score", "l8_movement_score", "l9_structure_score", "component_summary", "reason", "meaning", "directional_validity", "expectancy_validated", "selection_runtime", "trade_permission", "entry_signal"]
    return "\n".join([f"{key}={data.get(key, 'not_available')}" for key in keys] + [f"generated_utc={utc_stamp()}", f"generated_unix={unix_time()}", ""])
...
def _publish(outbox_root: Path, input_rows: List[Dict[str, str]], ranked_rows: List[Dict[str, str]]) -> L11PublishSummary:
    ...
    layer_dir.mkdir(parents=True, exist_ok=True); group_dir.mkdir(parents=True, exist_ok=True); symbol_dir.mkdir(parents=True, exist_ok=True)
    failed: List[Path] = []
    ...
    top5_rows = [dict(row, top_rank=row.get("ranking_group_rank", "not_available"), group_state="l11_ranked_view", rankable_count=row.get("ranking_group_rankable_count", "0"), source_ranked_symbols_checksum=ranked_checksum) for row in ranked_rows if row.get("top5_per_ranking_group") == "true"]
    top5_csv = _csv_text(top5_rows, L11_TOP5_FIELDS)
    lines = ["LAYER 11 - TOP 5 PER RANKING_GROUP", "----------------------------------------"]
    current_group = ""
    for row in top5_rows:
        if row.get("ranking_group") != current_group:
            current_group = row.get("ranking_group", "Unknown")
            lines += ["", f"Ranking Group: {current_group}"]
        lines.append(f"{row.get('ranking_group_rank')}. {row.get('symbol')} | L11 {row.get('l11_group_score')} | {row.get('component_summary')} | {row.get('rank_state')} | trade_permission=false")
    top5_text = "\n".join(lines + ["", "Meaning: intra_group_inspection_priority_only", "Selection Runtime: FALSE", "Trade Permission: FALSE", "Entry Signal: FALSE", ""])
    ...
    for group in sorted(set(row.get("ranking_group", "Unknown") for row in ranked_rows)):
        ...
        _write(group_dir / f"{slug}.ranked_symbols.csv", _csv_text(group_rows, L11_RANKED_FIELDS), failed)
        group_top5 = [row for row in group_rows if row.get("top5_per_ranking_group") == "true"]
        _write(group_dir / f"{slug}.top5.txt", "\n".join([f"{r.get('ranking_group_rank')}. {r.get('symbol')} | L11 {r.get('l11_group_score')} | {r.get('component_summary')} | trade_permission=false" for r in group_top5] + [""]), failed)
    symbol_written = 0
    ...
    summary = L11PublishSummary("accepted" if not failed and symbol_actual == symbol_written else "write_degraded", "l11_symbol_ranking_outputs_published" if not failed and symbol_actual == symbol_written else "one_or_more_l11_outputs_failed_or_symbol_sidecar_count_mismatch", len(input_rows), len(set(r.get("ranking_group", "Unknown") for r in ranked_rows)), ranked_count, sum(1 for r in ranked_rows if r.get("rank_state") == "ranked_partial"), sum(1 for r in ranked_rows if r.get("rank_state") == "risk_review"), sum(1 for r in ranked_rows if r.get("rank_state") == "not_rankable_taxonomy"), sum(1 for r in ranked_rows if r.get("rank_state") == "not_rankable_quality"), len(set(r.get("ranking_group", "Unknown") for r in top5_rows)), len(top5_rows), symbol_written, symbol_actual, "sanitized_symbol__payload_checksum", len(failed), str(layer_dir / "ranked_symbols_by_group.csv"), str(layer_dir / "ranking_group_top5.csv"), str(layer_dir / "l11_symbol_ranking_summary.txt"))
    _write(layer_dir / "l11_symbol_ranking_summary.txt", _summary_text(summary), failed)
    return summary
...
def publish_l11_symbol_ranking_inside_group(outbox_root: Path) -> L11PublishSummary:
    ...
    try:
        ...
        render_rows: Dict[str, Dict[str, Dict[str, str]]] = {}
        for key in ("l6", "l7", "l8", "l9"):
            render_rows[key] = {_safe_text(row, "symbol", ""): row for row in _read_csv(_render_index_path(outbox_root, key)) if _safe_text(row, "symbol", "")}
        ...
        for symbol in sorted(set().union(*(set(layer.keys()) for layer in render_rows.values()))):
            tax = taxonomy.get(symbol, {"symbol": symbol, "canonical_symbol": symbol, "asset_class": "Unknown", "market_group": "Unknown", "market_segment": "Unknown", "ranking_group": "Unknown", "taxonomy_state": "UNKNOWN", "review_state": "manual_review_required", "rank_allowed": "false", "selection_allowed": "false"})
            ...
            score, average, missing_penalty, stale_penalty, risk_penalty, component_summary = _score(components, rank_state)
            ranking_group = _safe_text(tax, "ranking_group", "Unknown")
            input_row = {"symbol": symbol, "canonical_symbol": _safe_text(tax, "canonical_symbol", symbol), "asset_class": _safe_text(tax, "asset_class", "Unknown"), "market_group": _safe_text(tax, "market_group", "Unknown"), "market_segment": _safe_text(tax, "market_segment", "Unknown"), "ranking_group": ranking_group, "ranking_group_slug": group_slugs.get(ranking_group, _sanitize(ranking_group)), "taxonomy_state": _safe_text(tax, "taxonomy_state", "UNKNOWN"), "review_state": _safe_text(tax, "review_state", "not_available"), "rank_allowed": _safe_text(tax, "rank_allowed", "false"), "selection_allowed": _safe_text(tax, "selection_allowed", "false"), "l5_gate_state": _safe_text(tax, "l5_gate_state", "not_available"), "l5_eligible_flag": _safe_text(tax, "l5_eligible_flag", "not_available"), "component_available_count": str(available_count), "component_missing_count": str(4 - available_count), "input_quality_state": "complete" if available_count == 4 else ("partial" if available_count >= 2 else "degraded"), "rank_eligibility_state": rank_state, "rank_eligibility_reason": reason, "selection_runtime": "false", "trade_permission": "false", "entry_signal": "false"}
            for key in ("l6", "l7", "l8", "l9"):
                item = components[key]
            ...
            base_rows.append({"ranking_group": ranking_group, "ranking_group_slug": input_row["ranking_group_slug"], "symbol": symbol, "canonical_symbol": input_row["canonical_symbol"], "asset_class": input_row["asset_class"], "market_group": input_row["market_group"], "market_segment": input_row["market_segment"], "l11_group_score": f"{score:.2f}", "rank_state": rank_state, "risk_review_flag": "true" if rank_state == "risk_review" else "false", "not_rankable_reason": reason if rank_state.startswith("not_rankable") else "not_applicable", "component_available_count": str(available_count), "component_missing_count": str(4 - available_count), "weighted_available_average": f"{average:.2f}", "missing_layer_penalty": f"{missing_penalty:.2f}", "stale_layer_penalty": f"{stale_penalty:.2f}", "risk_review_penalty": f"{risk_penalty:.2f}", "l6_cost_score": components["l6"]["normalized_score"], "l6_weight": components["l6"]["weight"], "l6_state": components["l6"]["rank_state"], "l7_session_score": components["l7"]["normalized_score"], "l7_weight": components["l7"]["weight"], "l7_state": components["l7"]["rank_state"], "l8_movement_score": components["l8"]["normalized_score"], "l8_weight": components["l8"]["weight"], "l8_state": components["l8"]["rank_state"], "l9_structure_score": components["l9"]["normalized_score"], "l9_weight": components["l9"]["weight"], "l9_state": components["l9"]["rank_state"], "component_summary": component_summary, "reason": reason, "meaning": "intra_group_inspection_priority_only", "directional_validity": "false", "expectancy_validated": "false", "selection_runtime": "false", "trade_permission": "false", "entry_signal": "false"})
