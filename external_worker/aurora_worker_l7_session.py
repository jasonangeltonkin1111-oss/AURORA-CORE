...
SESSION_PROFILE_SOURCE = "aurora_static_fx_session_profile_v1"
SESSION_PROFILE_POLICY = "static_gateway_profile_v2_dead_time_is_cautionary_not_trade_window"
NOT_AVAILABLE_RANGE_STATUS = "not_available_no_l7_ohlc_or_session_range_owner"
...
RANK_STATE_ORDER = {
    ...
    "ranked_partial": 3,
    "ranked_degraded": 2,
    "not_rankable_quality": 0,
}

OUTPUT_FIELDS = [
    "rank_index", "session_relevance_rank", "symbol", "layer_id", "layer_name",
    "session_score", "current_session_relevance_score", "session_relevance_score",
    "session_relevance_confidence", "session_bucket", "rank_state", "score_quality",
    "current_session", "current_session_name", "session_minutes_elapsed",
    "session_minutes_remaining", "session_definition_source", "session_time_basis",
    "time_basis_confidence", "symbol_session_fit_score", "session_activity_score",
    "live_activity_quality_score", "quote_freshness_quality_score", "session_spread_score",
    "spread_session_safety_score", "session_range_score", "session_range_score_status",
    "asset_class", "ranking_group", "market_state", "quote_quality", "surface_quality",
    "tick_age_seconds", "spread_bps", "daily_change_pct", "zero_spread_state",
...
def _session_minutes(seconds: int, session_name: str) -> Tuple[int, int]:
    ...
    elapsed = max(0, seconds - start) // 60
    remaining = max(0, end - seconds) // 60
...
def _time_basis_confidence(session_time_basis: str, session_definition_source: str) -> float:
    ...
    if "l4_refresh_time" in text:
        ...
        confidence = 30.0
    source = session_definition_source.lower()
    if source in {"pending_gateway_static_profile", SESSION_PROFILE_SOURCE} or "static" in source:
...
def _score_row(row: Dict[str, str]) -> Dict[str, str | float]:
    ...
    raw_session_source = _safe_text(row, "session_definition_source", SESSION_PROFILE_SOURCE)
    session_definition_source = SESSION_PROFILE_SOURCE if raw_session_source == "pending_gateway_static_profile" else raw_session_source
    ...
    rank_state = "ranked_partial" if session_definition_source == SESSION_PROFILE_SOURCE else "ranked"
    ...
    reasons: List[str] = [
        ...
        "static_gateway_profile_dead_time_caution_not_edge_proof",
    ]
    ...
    return {
        "symbol": symbol,
        "session_score": score,
        "current_session_relevance_score": score,
        "session_relevance_score": score,
        "session_relevance_confidence": session_confidence,
        "session_bucket": bucket,
        "rank_state": rank_state,
        "score_quality": score_quality,
        "current_session": current_session,
        "current_session_name": current_session,
        "session_minutes_elapsed": float(minutes_elapsed),
        "session_minutes_remaining": float(minutes_remaining),
        "session_definition_source": session_definition_source,
        "session_time_basis": session_time_basis,
        "time_basis_confidence": time_basis_confidence,
        "symbol_session_fit_score": fit_score,
        "session_activity_score": activity_score,
        "live_activity_quality_score": activity_score,
        "quote_freshness_quality_score": quote_score,
        "session_spread_score": spread_score,
        "spread_session_safety_score": spread_score,
        "session_range_score": "not_available",
        "session_range_score_status": NOT_AVAILABLE_RANGE_STATUS,
...
def _top20_text(scored: List[Dict[str, str | float]]) -> str:
    lines = [
        "LAYER 7 - SESSION RELEVANCE RANKING - TOP 20",
        "----------------------------------------",
        ...
        "",
        "rank|symbol|score|confidence|bucket|state|session|elapsed_min|remaining_min|reason",
    ]
    for index, row in enumerate(scored[:20], start=1):
        lines.append(
            f"{index}|{row['symbol']}|{float(row['session_score']):.2f}|"
            f"{float(row['session_relevance_confidence']):.2f}|{row['session_bucket']}|"
            f"{row['rank_state']}|{row['current_session']}|{int(float(row['session_minutes_elapsed']))}|"
            f"{int(float(row['session_minutes_remaining']))}|{_bounded_reason(str(row['reason']))}"
        )
    lines.append("")
    return "\n".join(lines)
...
def _symbol_rank_text(rank_index: int, row: Dict[str, str | float]) -> str:
    ...
    lines = [
        ...
        f"session_relevance_rank={rank_index}",
        f"symbol={symbol}",
        ...
        f"current_session_relevance_score={float(row['current_session_relevance_score']):.6f}",
        f"session_relevance_score={float(row['session_relevance_score']):.6f}",
        f"session_relevance_confidence={float(row['session_relevance_confidence']):.6f}",
        ...
        f"current_session_name={row['current_session_name']}",
        f"session_minutes_elapsed={int(float(row['session_minutes_elapsed']))}",
        f"session_minutes_remaining={int(float(row['session_minutes_remaining']))}",
        ...
        f"session_activity_score={float(row['session_activity_score']):.6f}",
        ...
        f"session_spread_score={float(row['session_spread_score']):.6f}",
        f"spread_session_safety_score={float(row['spread_session_safety_score']):.6f}",
        f"session_range_score={row['session_range_score']}",
        f"session_range_score_status={row['session_range_score_status']}",
        ...
        f"reason={_format_value(_bounded_reason(str(row['reason'])))}",
...
def _manifest(summary: L7RankSummary, input_path: Path) -> str:
    ...
    return "\n".join([
        ...
        f"stale_final_files_removed={summary.stale_final_files_removed}",
        f"stale_final_files_failed={summary.stale_final_files_failed}",
        ...
        "selection_runtime=false",
        "publication_order=write_expected_outputs_then_delete_stale_then_manifest_last",
        f"session_profile_policy={SESSION_PROFILE_POLICY}",
        "session_range_score_policy=not_available_until_non_l7_range_owner_provides_source",
        f"reason_max_parts={L7_REASON_MAX_PARTS}",
        f"reason_max_chars={L7_REASON_MAX_CHARS}",
        f"generated_utc={utc_stamp()}",
        f"generated_unix={unix_time()}",
        "",
    ])
...
def _summary_from_ranked_manifest(manifest_text: str, fallback: L7RankSummary) -> L7RankSummary:
    ...
    return L7RankSummary(
        ...
        reason=_bounded_reason(data.get("reason", fallback.reason)),
        ...
        stale_final_files_removed=fallback.stale_final_files_removed,
        stale_final_files_failed=fallback.stale_final_files_failed,
...
def _layer_paths(outbox: Path) -> Tuple[Path, Path, Path, Path, Path, Path]:
    ...
    return (
        layer_dir,
        layer_dir / L7_INPUT_NAME,
        layer_dir / L7_INPUT_MANIFEST_NAME,
        layer_dir / L7_RANKED_NAME,
        layer_dir / L7_MANIFEST_NAME,
        layer_dir / L7_TOP20_NAME,
    )
...
def publish_l7_session_relevance_rankings(outbox: Path) -> L7RankSummary:
    ...
    summary = L7RankSummary(
        "missing_input",
        "l7_input_primitives.csv missing",
    ...
    scored.sort(
        key=lambda row: (
            RANK_STATE_ORDER.get(str(row["rank_state"]), 0),
            SESSION_BUCKET_ORDER.get(str(row["session_bucket"]), 0),
            float(row["session_score"]),
            float(row["symbol_session_fit_score"]),
            float(row["live_activity_quality_score"]),
            str(row["symbol"]),
        ),
        reverse=True,
    )
    ...
    if not summary.input_generation_stable:
        ...
        summary.reason = (
            f"l7 input changed while ranking; before_count={summary.input_count}; "
            f"after_count={len(after_rows)}; before_checksum={summary.input_payload_checksum}; "
            f"after_checksum={summary.input_payload_checksum_after_rank}; existing ranked outputs intentionally left untouched"
        )
        atomic_write_text(manifest_path, _manifest(summary, input_path))
        return summary
