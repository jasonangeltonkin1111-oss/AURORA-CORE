...
$statusDir = Join-Path $root "Gateway\Status"
$legacyStatusDir = Join-Path $root "External Worker\Status"
if (!(Test-Path $statusDir) -and (Test-Path $legacyStatusDir)) { $statusDir = $legacyStatusDir }
...
Write-Host "Gateway process count: $procCount"
Write-Host "gateway_operator_cmd_required(actual)=$operatorRequired"
Write-Host "gateway_status_folder=$statusDir"
