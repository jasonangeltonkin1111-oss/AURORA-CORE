...
check_has l11_fn_board "${REQ[6]}" "AC_Layer11BoardSection\\("
check_has l11_fn_dossier "${REQ[6]}" "AC_Layer11DossierSection\\("
check_has l11_fn_workbench "${REQ[6]}" "AC_Layer11WorkbenchSection\\("
check_has marketboard_calls_l11 "${REQ[7]}" "AC_Layer11BoardSection\\("
check_has workbench_calls_l11 "${REQ[7]}" "AC_Layer11WorkbenchSection\\("
check_has publication_includes_l11 "${REQ[9]}" "AC_Layer11SelectionGroupsRenderer.mqh"
check_has dossier_composition_wrapper "${REQ[9]}" "AC_Layer11AndSharedOhlcRenderDossierSection"
check_has dossier_wrapper_appends_l11 "${REQ[9]}" "AC_Layer11DossierSection\\(symbol\\)"
check_has dossier_macro_bridge "${REQ[9]}" "#define AC_SharedOhlcRenderDossierSection AC_Layer11AndSharedOhlcRenderDossierSection"

for f in external_worker/aurora_worker_l11.py external_worker/aurora_worker_l11_cleanup.py external_worker/aurora_worker_l11_dispatch.py external_worker/aurora_worker_l11_tree.py mt5/runtime_owners/runtime_7_publication_owner/publication_renderers/AC_Layer11SelectionGroupsRenderer.mqh; do
...
if [[ -z "${APPDATA:-}" ]]; then
  ...
  rank_card="$(find "$COMMON_ROOT" -path '*/Selection Desk/Groups/*/*/*/*/[0-9][0-9]_*.txt' -type f | head -n 1 || true)"
  ...
  [[ -n "$rank_card" ]] && pass taxonomy_rank_card "$rank_card" || fail taxonomy_rank_card "missing"
fi
