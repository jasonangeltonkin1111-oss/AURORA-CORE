...
static const string AC_BUILD_PHASE        = "surface_dossier_catchup_acceleration";
static const string AC_BUILD_VERSION      = "1.066";
static const string AC_UPGRADE_ID         = "SURFACE_DOSSIER_CATCHUP_ACCELERATION";
static const string AC_UPGRADE_SUMMARY    = "Accelerates bounded Dossier universe catch-up after L6-L8 surface ranking changes so Market Board, Workbench, and symbol Dossiers converge faster. This is a config/schema refresh only: no duplicate Dossier writer, no new FileIO route owner, no L6-L8 scoring math change, no trade permission, no selection permission, and no execution permission is added.";
static const string AC_UPGRADE_SCOPE      = "Dossier surface cadence/coherence patch only. Runtime 1 remains source truth for Layers 1-5. Runtime 3/Gateway remains calculation_support_only for Layer 6+ support. Runtime 7 remains the only Board/Dossier/FileIO publication owner. The full priority-open-Dossier refresh lane still requires a source-owner patch in AC_Layer0DossierPublication.mqh; this config patch only improves catch-up speed and forces schema refresh.";
static const string AC_UPGRADE_TEST_PLAN  = "No compile/runtime proof is claimed by source patch alone. Compile must confirm build_version=1.066. Runtime proof must confirm dossier_schema=dossier_v1.066_surface_dossier_catchup_acceleration, dossier batch status, batch_duration_ms below timer budget, L7/L8 accepted on Market Board, and open/pass Dossiers converging to accepted_current_epoch within fewer bounded passes than v1.065.";
static const string AC_LOGGING_POLICY     = "event_boundary_surface_dossier_catchup_acceleration_no_duplicate_owner_no_permission";
...
static const string AC_DOSSIER_SHELL_SCHEMA_VERSION = "dossier_v1.066_surface_dossier_catchup_acceleration";
