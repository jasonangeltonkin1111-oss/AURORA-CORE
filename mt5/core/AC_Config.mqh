...
static const string AC_BUILD_VERSION      = "1.072";
static const string AC_UPGRADE_ID         = "SELECTION_READBACK_AND_TIMER_PRESSURE_REPAIR";
static const string AC_UPGRADE_SUMMARY    = "Repairs MT5 readback of current worker selection surfaces by accepting canonical Selection Desk outputs and preventing pending scaffold overwrite of accepted L16/L17 surfaces. Retunes the MT5 timer cadence from a 250ms experimental heartbeat to a sane 5s cadence because runtime evidence showed publication passes taking tens of seconds; MT5 timer events are not queue-backed while a prior timer pass is still running.";
static const string AC_UPGRADE_SCOPE      = "Runtime 7 publication/render readback surfaces plus Runtime 0 timer cadence config. No trade permission, execution, broker polling, FileIO owner, route owner, setup owner, or new worker owner is added.";
static const string AC_UPGRADE_TEST_PLAN  = "Compile must confirm build_version=1.072. Runtime proof must include Market Board showing current L16/L17 worker outputs instead of pending scaffold, Selection Desk status schema v3, no overwrite of accepted 01_Global Top_10 and Deep_Evidence surfaces, and Runtime Status showing timer_period_ms=5000. No packet import, matching, live capture, permission, or execution is expected.";
static const string AC_LOGGING_POLICY     = "event_boundary_selection_readback_timer_pressure_no_permission_no_new_owner";
...
static const string AC_DOSSIER_SHELL_SCHEMA_VERSION = "dossier_v1.072_selection_readback_timer_pressure_repair";
...
static const int    AC_TIMER_MILLISECONDS = 5000;
static const int    AC_WORKBENCH_INTERVAL_HEARTBEATS = 12;
