...
static const string AC_BUILD_PHASE        = "external_worker_foundation";
static const string AC_BUILD_VERSION      = "1.037";
static const string AC_UPGRADE_ID         = "EXTERNAL_WORKER_FOUNDATION";
static const string AC_UPGRADE_SUMMARY    = "Adds Runtime 3 External Calculation Worker foundation: Workbench-only worker control/status, required-worker request file, expected package paths, worker heartbeat/result placeholders, and sealed sidecar boundary before Layer 5 heavy calculations.";
static const string AC_UPGRADE_SCOPE      = "Runtime 3 owns worker relationship only: install/status detection, worker_required control output, heartbeat/result path contracts, and Workbench diagnostics. It does not own broker truth, L1-L4 truth, FileIO internals, Board/Dossier rendering authority, ranking, selection, permission, strategy, execution, WebRequest, Python direct broker authority, or popup alerts.";
static const string AC_UPGRADE_TEST_PLAN  = "Compile AuroraCore.mq5; run EA; confirm Workbench Status includes External Worker section, worker_required.txt is written under Workbench/External Worker/Control, missing worker package is reported as Workbench-only degraded state, Market Board/Dossiers continue printing, and trade_permission remains false.";
static const string AC_LOGGING_POLICY     = "near_instant_board_write_if_changed_plus_external_worker_workbench_status_no_popup_alerts_no_timer_blocking";
...
static const string AC_DOSSIER_SHELL_SCHEMA_VERSION = "dossier_v1.037_external_worker_foundation";
