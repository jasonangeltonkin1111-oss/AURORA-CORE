...
// Purpose: selected-symbol rolling MT5 tick-window/spread-observation proxy metrics.
// L4 remains the source owner for current bid, ask, last, live spread, and quote freshness.
// L20 may store historical MqlTick row observations internally only to derive selected rolling-window metrics.
// Forbidden: current quote ownership, institutional order flow, buy/sell pressure, setup confirmation, trade permission, execution, DOM, ATR/VWAP/indicator ownership.
...
double AC_L20_ROW_OBS_VOLUME_REAL[AC_L20_MAX_SELECTED_SYMBOLS][AC_L20_MAX_TICK_ROWS_PER_SYMBOL];
...
{
   return "selected MT5 tick-row observations only; current_quote_owner=L4; no signal or permission";
}
...
{
   for(int i = 0; i < AC_L20_MAX_SELECTED_SYMBOLS; i++)
      if(AC_L20_SYMBOLS[i] == symbol) return i;
   return -1;
}
...
void AC_L20RetireMissingSelections(string &selected_symbols[], const int selected_count, const ulong now_msc)
{
   for(int slot = 0; slot < AC_L20_MAX_SELECTED_SYMBOLS; slot++)
   {
      ...
      {
         if(selected_symbols[i] == AC_L20_SYMBOLS[slot]) { still_selected = true; break; }
      }
...
void AC_L20AppendTickRowToSlot(const int slot, const MqlTick &tick, const double point)
{
   ...
   if(count >= AC_L20_MAX_TICK_ROWS_PER_SYMBOL)
   {
      for(int i = 1; i < count; i++)
      {
         ...
         AC_L20_ROW_OBS_VOLUME_REAL[slot][i - 1] = AC_L20_ROW_OBS_VOLUME_REAL[slot][i];
   ...
   AC_L20_ROW_OBS_VOLUME_REAL[slot][count] = tick.volume_real;
...
int AC_L20PruneOldTickRows(const int slot, const ulong cutoff_msc)
{
   ...
   for(int i = first_keep; i < count; i++)
   {
      ...
      AC_L20_ROW_OBS_VOLUME_REAL[slot][new_count] = AC_L20_ROW_OBS_VOLUME_REAL[slot][i];
...
void AC_L20EmptySummary(AC_L20SymbolSummary &summary, const string symbol, const string status, const string reason)
{
   ...
   summary.l4_quote_reference_status = "not_wired";
   ...
   summary.current_quote_owner = false;
...
void AC_L20SummarizeSlot(const int slot, const ulong now_msc, AC_L20SymbolSummary &summary)
{
   ...
   AC_L20EmptySummary(summary, AC_L20_SYMBOLS[slot], "ACTIVE_ROLLING", "rolling_tick_window_summarized");
   summary.selected_scope_source = "selected_scope_owner_pending_runtime_wiring";
   summary.l4_quote_reference_status = "pending_L4_reference_reader";
   ...
   if(count <= 0)
   {
      ...
      summary.reason = "no_tick_rows_in_buffer";
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      if(!mid_seeded) { mid_min = mid; mid_max = mid; mid_seeded = true; }
      else { if(mid < mid_min) mid_min = mid; if(mid > mid_max) mid_max = mid; }
      ...
      if(i > 0)
      {
         ...
         if(AC_L20_ROW_OBS_BID[slot][i] > AC_L20_ROW_OBS_BID[slot][i - 1]) summary.bid_up_count_10m++;
         if(AC_L20_ROW_OBS_BID[slot][i] < AC_L20_ROW_OBS_BID[slot][i - 1]) summary.bid_down_count_10m++;
         if(AC_L20_ROW_OBS_ASK[slot][i] > AC_L20_ROW_OBS_ASK[slot][i - 1]) summary.ask_up_count_10m++;
         if(AC_L20_ROW_OBS_ASK[slot][i] < AC_L20_ROW_OBS_ASK[slot][i - 1]) summary.ask_down_count_10m++;
   ...
   summary.oldest_tick_msc = AC_L20_ROW_TIME_MSC[slot][0];
   summary.newest_tick_msc = AC_L20_ROW_TIME_MSC[slot][count - 1];
   ...
   if(summary.tick_count_10m <= 0)
   {
      ...
      summary.reason = "no_tick_rows_in_rolling_window";
      summary.sample_quality = "none";
      summary.proxy_confidence = "none";
   }
   else if(summary.latest_tick_age_seconds > 60)
   {
      ...
      summary.reason = "latest_tick_row_too_old";
   ...
   else if(summary.spread_spike_severe_count_10m > 0)
   {
      ...
      summary.reason = "severe_observed_spread_spike_seen";
   ...
   else
   {
      ...
      summary.reason = "rolling_tick_window_proxy_active";
...
{
   return "symbol,status,reason,selected_scope_source,l4_quote_reference_status,tick_row_count_1m,tick_row_count_5m,tick_row_count_10m,spread_observed_min_points_10m,spread_observed_avg_points_10m,spread_observed_max_points_10m,spread_observed_stddev_points_10m,spread_observed_spike_count_10m,tick_gap_avg_seconds,tick_gap_max_seconds,observed_bid_change_count_10m,observed_ask_change_count_10m,observed_last_change_count_10m,observed_volume_change_count_10m,observed_bid_up_count_10m,observed_bid_down_count_10m,observed_ask_up_count_10m,observed_ask_down_count_10m,observed_mid_change_count_10m,observed_mid_range_points_10m,latest_tick_row_age_seconds,buffer_count,oldest_tick_msc,newest_tick_msc,flags_decode_status,sample_quality,proxy_confidence,current_quote_owner,institutional_order_flow_claim,trade_permission,entry_signal,execution";
}
...
string AC_L20DossierSection(const AC_L20SymbolSummary &s)
{
   ...
   text += "Source: MT5 historical tick rows; current quote owner=L4\r\n";
   text += "L4 Reference: " + s.l4_quote_reference_status + "\r\n";
   ...
   text += "Observed Row Changes: bid=" + IntegerToString(s.bid_change_count_10m) + " | ask=" + IntegerToString(s.ask_change_count_10m) + " | last=" + IntegerToString(s.last_change_count_10m) + " | volume=" + IntegerToString(s.volume_change_count_10m) + "\r\n";
   text += "Observed Mid Proxy: changes=" + IntegerToString(s.mid_change_count_10m) + " | range_points=" + DoubleToString(s.mid_range_points_10m, 2) + "\r\n";
   text += "Quality: sample=" + s.sample_quality + " | confidence=" + s.proxy_confidence + " | flags=" + s.flags_decode_status + "\r\n";
   text += "Boundary: L20 is review-only tick-window context; L4 owns current quote truth; no signal or permission\r\n";
   return text;
}
