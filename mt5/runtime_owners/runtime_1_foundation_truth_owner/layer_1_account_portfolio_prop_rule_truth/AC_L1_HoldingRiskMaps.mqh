#ifndef AC_L1_HOLDING_RISK_MAPS_MQH
#define AC_L1_HOLDING_RISK_MAPS_MQH

string AC_L1HoldingRiskLine(const string bucket,
                            const int rows,
                            const int wins,
                            const int losses,
                            const double net,
                            ...
                            const int extreme_breaches)
{
   ...
   return AC_L1PadRight(bucket, 10)
...
string AC_L1HoldingTimeRiskMapV2()
{
   int rows[5];
   int wins[5];
   int losses[5];
   ...
   for(int i = 0; i < 5; i++)
   {
      ...
      if(first_weak || nor < weakest_net_over_risk)
      {
         ...
         weakest_bucket = AC_L1HoldBucketName(i);
      }
   }
   ...
   text += "Hold Unavailable Rows:  " + IntegerToString(hold_unavailable_rows) + "\r\n";
   text += "Weakest Net/Risk Hold:  " + weakest_bucket + " " + DoubleToString(weakest_net_over_risk, 2) + "\r\n";
   text += AC_L1PadRight("Hold", 10)
      + AC_L1PadLeft("Rows", 6)
      + AC_L1PadLeft("Win%", 9)
      + AC_L1PadLeft("Net", 11)
   ...
   for(int i = 0; i < 5; i++)
   {
      ...
      text += AC_L1HoldingRiskLine(AC_L1HoldBucketName(i), rows[i], wins[i], losses[i], net[i], risk[i], net_r[i], br, wr, unit_breaches[i], hard_breaches[i], extreme_breaches[i]);
   }
