                              ...
                              const string value)
{
   return AC_L1PadRight(severity, 9)
      + "| " + AC_L1PadRight(AC_L1DisplayText(key), 38)
      + "| " + AC_L1DisplayText(value)
      + "\r\n";
}
...
string AC_L1DataQualityLedger()
{
   ...
   text += AC_L1QualityLedgerLine("INFO", "Selected history rows", IntegerToString(rows_total));
   text += AC_L1QualityLedgerLine(rows_without_entry > 0 ? "WARNING" : "OK", "Rows without entry truth", IntegerToString(rows_without_entry));
   text += AC_L1QualityLedgerLine(rows_without_sl > 0 ? "WARNING" : "OK", "Rows without stop loss", IntegerToString(rows_without_sl));
   text += AC_L1QualityLedgerLine(rows_without_tp > 0 ? "INFO" : "OK", "Rows without take profit", IntegerToString(rows_without_tp));
   text += AC_L1QualityLedgerLine(rows_invalid_risk_geometry > 0 ? "WARNING" : "OK", "Rows invalid SL geometry", IntegerToString(rows_invalid_risk_geometry));
   text += AC_L1QualityLedgerLine(rows_partial_reconstruction > 0 ? "WARNING" : "OK", "Rows partial entry reconstruction", IntegerToString(rows_partial_reconstruction));
   text += AC_L1QualityLedgerLine(rows_order_context_partial > 0 ? "INFO" : "OK", "Rows order context partial", IntegerToString(rows_order_context_partial));
   text += AC_L1QualityLedgerLine(rows_order_context_unavailable > 0 ? "WARNING" : "OK", "Rows order context unavailable", IntegerToString(rows_order_context_unavailable));
   text += AC_L1QualityLedgerLine(rows_cost_allocated_partial > 0 ? "INFO" : "OK", "Rows partial cost allocation source", IntegerToString(rows_cost_allocated_partial));
   text += AC_L1QualityLedgerLine(rows_magic_zero > 0 ? "INFO" : "OK", "Rows magic zero", IntegerToString(rows_magic_zero));
   text += AC_L1QualityLedgerLine(rows_comment_missing > 0 ? "INFO" : "OK", "Rows comment missing", IntegerToString(rows_comment_missing));
   text += AC_L1QualityLedgerLine(rows_large_loss_over_hard_risk > 0 ? "CRITICAL" : "OK", "Loss rows over 0.2% hard risk", IntegerToString(rows_large_loss_over_hard_risk));
   text += AC_L1QualityLedgerLine(cluster_rows > 0 ? "WARNING" : "OK", "Cluster rows same symbol side time", IntegerToString(cluster_rows) + " rows / " + IntegerToString(cluster_groups) + " groups / net " + AC_L1MoneyText(cluster_net));
   text += AC_L1QualityLedgerLine("INFO", "Ledger scope", "Diagnostic only - no trade permission");
   return text;
}
