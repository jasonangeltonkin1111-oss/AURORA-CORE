...
void OnTimer()
{
   ...
   start = GetTickCount();
   AC_WriteResult manifest = AC_PublishManifest(account_status, worker_required, board);
   ...
   start = GetTickCount();
   AC_WriteResult diagnostics = AC_PublishDiagnostics(account_status, worker_required, board);
