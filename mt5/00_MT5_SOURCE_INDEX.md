...
- Layer 4 source is active as Market Watch / live quote / spread truth owner. It remains the single owner for current bid, ask, last, live spread, and quote freshness.
...
- Runtime 5 Layer 20 source is present as selected rolling tick-window metric scaffold only. It is not included by `mt5/AuroraCore.mq5`, not compile-proven, not runtime-proven, and cannot merge until L19 is confirmed running on main. L20 must not become current quote owner; L4 remains current bid/ask/last/live spread/quote freshness owner.
...
- L20 may use `CopyTicksRange`/`MqlTick` only for already-selected symbols after deliberate activation.
- L20 owns selected rolling tick-window metrics only. It does not own current bid, ask, last, live spread, or quote freshness.
- L4 remains the source owner for current Market Watch quote/spread/freshness truth.
